...
def create_viral_short(
    ...
    Flow:
    1. Download video section using yt-dlp (headless, cloud-safe)
    2. Apply vertical mastering with FFmpeg (9:16 crop)
    3. Return final output path
    """
    ...
    # Format timestamps for yt-dlp section spec
    start_ts = format_hhmmss(start_seconds)
    ...
    if not os.path.exists(temp_clip_path) or os.path.getsize(temp_clip_path) == 0:
        if status_placeholder:
            if last_error and "403" in last_error:
                status_placeholder.error("❌ HTTP 403 Forbidden: YouTube is blocking this Streamlit Cloud server IP. This is a rate-limit or geographic restriction by YouTube.")
            else:
                status_placeholder.error("❌ Download blocked on Streamlit Cloud. Server may be rate-limited or restricted.")
        if last_error:
    ...
    if status_placeholder:
        status_placeholder.info("✂️ Extracting & mastering 9:16 vertical...")
    
    result = render_short_clip_ffmpeg(
        temp_clip_path,
        0.0,
        duration,
        output_file,
